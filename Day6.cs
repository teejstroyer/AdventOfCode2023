...
Distance:  9  40  200 ";
    ...
    public static void Run()
    {
        ...
        //Merge above into tuple list
        var raceData = times.Zip(distances, (time, distance) => (time: time, distance: distance, speed: (double)distance / (double)time)).ToList();

        int product = CalculateWaysToWinProduct(raceData);

        Console.WriteLine($"Part 1: {product}");

        var time = long.Parse(times.Select(i => i.ToString()).Aggregate((a, b) => a + b));
        var distance = long.Parse(distances.Select(i => i.ToString()).Aggregate((a, b) => a + b));
        var speed = (double)distance / (double)time;

        product = CalculateWaysToWinProduct([(time: time, distance: distance, speed: speed)]);
        Console.WriteLine($"Part 2: {product}");
    }

    private static int CalculateWaysToWinProduct(List<(long time, long distance, double speed)> raceData)
    {
        int product = 1;
        foreach (var race in raceData)
        {
            ...
            //x^2 - Time * x + Distance = 0 //quadratic formula
            //hold = (Time +- sqrt(Time^2 - 4 * Distance)) / 2

            double b = race.time;
            double sqrt = Math.Sqrt(Math.Pow(b, 2) - 4 * race.distance);

            var max = (b + sqrt) / (2);
            var min = (b - sqrt) / (2);

            max = max % 1 == 0 ? max - 1 : Math.Floor(max);
            min = min % 1 == 0 ? min + 1 : Math.Ceiling(min);
